...
void LCD_Show_Chinese(u16 x,u16 y,u8 index,u16 color)
{
    u8 temp,t,t1;
    u16 y0=y; 
...
num为连续显示的个数
************************************************/
void LCD_ShowContent(u16 x,u16 y,u16 color,u16 t,u16 num)
{
    u8 i;
    for(i = t; i < t+num; i++)
    {
        LCD_Show_Chinese(x + (i-t)*16,y,i,color);
    }
}


//	从开头一直连续显示中文，直到把字库显示完
void LCD_ShowContent_up_down(u16 x,u16 y,u16 color)
{
    u8 i;
    for(i = 0; i < (sizeof(asc2_1616)/sizeof(asc2_1616[0])/2- 1); i++)
    {
        LCD_Show_Chinese(x + i*16,y,i,color);
    }
}
...
//中文界面设计
		POINT_COLOR=GREEN;	  
//		LCD_ShowString(10,5,210,24,24,"Hiki"); 
		LCD_ShowString(250,5,210,16,16,"THD:"); 
//		LCD_ShowString(150,10,200,16,16,"STM32"); 	
//	 	LCD_ShowContent(190,10,GREEN,11,3);
		LCD_ShowContent(270,30,RED,6,2);
		LCD_ShowContent(270,80,RED,0,3);
		LCD_ShowContent(270,130,RED,3,3);
		LCD_ShowContent(270,180,RED,8,3);
		POINT_COLOR=MAGENTA;
		LCD_ShowString(260,220,200,16,16,"ing...");
	
//白色框架 x=0~256		y=40~240		
		POINT_COLOR=WHITE;
		LCD_DrawLine(0, 40, 0, 240);
		LCD_DrawLine(256, 40, 256, 240);
		LCD_DrawLine(0, 40, 256, 40);
		LCD_DrawLine(0, 240, 256, 240);
...
{         	
    ...
    if(num < 0)
	{
        LCD_ShowChar(x,y,'0'- 3,size2);
		num =fabs(num);
	}
    else
	LCD_ShowChar(x,y,' ',size2);//第一位显示符号
	temp1 = (int)temp;
	temp2 = num - temp1;
	len1 = len - 6;//len1为整数部分位数，若显示数位需要扩展，修改该行
	LCD_ShowChar(x + size2/2*4,y,'0'- 2,size2);//浮点数的第5位显示小数点
	x = x + size2/2;
	...
	LCD_ShowChar(x+(size2/2)*4,y,((int)(temp2*10)%10) + '0',size2); //小数第一位
	LCD_ShowChar(x+(size2/2)*5,y,((int)(temp2*100)%10) + '0',size2); //小数第2位
	LCD_ShowChar(x+(size2/2)*6,y,((int)(temp2*1000)%10) + '0',size2); //小数第3位
	LCD_ShowChar(x+(size2/2)*7,y,((int)(temp2*10000)%10) + '0',size2); //小数第4位
}
