 ...
 {	 
	...
	while(1)
	{
		...
		for(i=0;i<NPT;i++)
		{
			...
			POINT_COLOR=BLUE;
			LCD_ShowxNum(280,100,adcmax*1.5,4,16,0);//显示电压最大值
			LCD_ShowxNum(280,150,adcmin*1.5,4,16,0);//显示电压最小值
			LCD_ShowxNum(280,200,(adcmax-adcmin)*1.5,4,16,0);//显示电压峰峰值
...
void GetPowerMag(void)
{
	...
	if(th==5)
	{
		...
		THDEND=THDEND/100;
		  sprintf(buffer,"%2.2f",THDEND);//浮点型数据转为指定格式的字符串
		  LCD_ShowString(280,10,200,16,16,buffer); //直接送字符显示函数
